...
export default defineConfig({
  ...
  description: "Join us at Code Corvallis, where programmers, coders, and software engineers in Corvallis come together to connect, share, and learn.",
  themeConfig: {
    nav: [
      { text: 'Home', link: '/' },
    ...
    socialLinks: [
      ...
      { icon: 'github', link: 'https://github.com/CodeCorvallis/CodeCorvallis' },
    ],
    footer: {
  ...
  transformHead({ siteConfig, pageData }) {
    ...
    const pageUrl = `https://codecorvallis.com/${pageData.relativePath.replace(/index\.md$/, '').replace(/\.md$/, '.html')}`

    // Generate keywords meta tags from tags array
    const tagMetaTags = tags.length > 0 ? [{
      name: 'keywords',
      content: tags.join(', ')
    }] : []
